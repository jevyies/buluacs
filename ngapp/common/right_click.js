angular.module('app').directive('contextMenu', [
	...
	function($parse, $q, custom, $sce) {
		...
		var processTextItem = function($scope, item, text, event, model, $promises, nestedMenu, $) {
			...
			var $a = $('<a>');
			$a.addClass('menu-right-click-option');
		...
		var processItem = function($scope, event, model, item, $ul, $li, $promises, $q, $, level) {
			...
			var nestedMenu =
				angular.isArray(item[1]) || (item[1] && angular.isFunction(item[1].then))
					? item[1]
		...
		var registerEnabledEvents = function(
			$scope,
			enabled,
			item,
			$ul,
		...
		var renderContextMenu = function($scope, event, options, model, level, customClass) {
			...
			var $ul = $('<ul>');
			$ul.addClass('dropdown-menu right-click-menu');
			$ul.attr({ role: 'menu' });
			$ul.css({
				display: 'block',
				position: 'absolute',
